...
export default {
  ...
  run: async (client, m, args, command, text, prefix) => {
    ...
    try {
      ...
      if (users.length === 0)
        return m.reply(`❂ No hay usuarios con más de 1,000 ${monedas}.`)
      ...
      await m.reply(textBoard)
    } catch {
      await m.reply('❂ Ocurrió un error al generar el ranking.')
    }
  }
}