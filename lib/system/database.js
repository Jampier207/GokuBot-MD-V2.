...
global.saveDatabase = function saveDatabase() {
  ...
  for (const [clave, valor] of Object.entries(settings)) {
    insertSetting.run(clave, safeStringify(valor))
  }

  global.db._snapshot.users = safeStringify(users)
  global.db._snapshot.chats = safeStringify(chats)
  global.db._snapshot.settings = safeStringify(settings)
}
