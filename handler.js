...
global.applyCooldown = function (baseTime, sender) {

  const user = global.db?.data?.users?.[sender]
  if (!user) return baseTime

  const now = Date.now()
  const isPremium = user.premium && user.premiumTime > now

  return isPremium ? Math.floor(baseTime / 2) : baseTime
}
...
const rawBotname = settings.namebot2 || 'GokuBot-MD'
const tipo = settings.type || 'Sub'
const isValidBotname = /^[\w\s]+$/.test(rawBotname)
const namebot = isValidBotname ? rawBotname : 'GokuBot-MD'
...
console.log(
chalk.hex('#BA55D3')('┌──────────────────────────────'),
chalk.hex('#FF69B4')(`\n│ 👤 Usuario   : `) + chalk.white(pushname),
m.isGroup
? chalk.hex('#FF69B4')(`\n│ 👥 Grupo     : `) + chalk.white(groupName)
: chalk.hex('#FF69B4')(`\n│ 💬 Chat      : `) + chalk.white('Privado'),
chalk.hex('#FF69B4')(`\n│ ⚡ Comando   : `) + chalk.cyan(command),
chalk.hex('#FF69B4')(`\n│ 🕒 Hora      : `) + chalk.yellow(moment().format('DD/MM/YY HH:mm:ss')),
chalk.hex('#BA55D3')('\n└──────────────────────────────\n')
)
}
...
if (chat.onlyowner) {
  ...
  const isBotOwner =
    global.db.data.settings[selfId]?.owner === m.sender

  if (!isOwnerGlobal && !isBotOwner) {
    return m.reply('✅ Este grupo está en modo *Solo Owner*.')
  }
}
...
if (user2.premium && user2.premiumTime && user2.premiumTime < Date.now()) {
  user2.premium = false
  user2.premiumTime = 0
}
  ...
  console.error(error)
  return m.reply(`[❌ Error *TypeError*]:\n${error.message}`)
}
   level(m)
};