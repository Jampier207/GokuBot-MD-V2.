export default {
  ...
  run: async (client, m) => {
    ...
    if (timeSinceLast > twoDays) {
      ...
      return m.reply(
        `◲ Has reclamado ${recompensa.toLocaleString()} ${monedas} (Día 1)\n` +
        `> Día 2 → ${siguiente.toLocaleString()} ${monedas}` +
    ...
    const rachaExtra = user.dailyStreak >= 10
      ? `\n> ◴ Racha de ${user.dailyStreak} días activa`
      : ''

    await m.reply(
      `◵ Has reclamado ${recompensa.toLocaleString()} ${monedas} (Día ${user.dailyStreak})\n` +
      `> Día ${user.dailyStreak + 1} → ${siguiente.toLocaleString()} ${monedas}${rachaExtra}`
    )
  },
}
